...
from KMapSolver import KMapSolver2, KMapSolver3, KMapSolver4
...
class KMapSolvers_GUI(QMainWindow, Ui_KMapSolvers):
    ...
    def solveKMap2(self):
        self.map_data = [[
            self.valueToInt(self.but_2__0_0.text()),
            self.valueToInt(self.but_2__0_1.text())
        ],
                         [
                             self.valueToInt(self.but_2__1_0.text()),
                             self.valueToInt(self.but_2__1_1.text())
                         ]]
        self.all_vars = 'A, B'
    ...
    def clear(self, p_int, cleanText=False):
        ...
        for member in members:
            memb = getattr(self, member)
            if isinstance(memb, QPushButton) and (
                    memb.objectName().startswith('KMap_' + str(p_int))
                    or memb.objectName().startswith('but_' + str(p_int))):
                memb.setStyleSheet('')
                if cleanText:
                    memb.setText('0')
        if cleanText:
    ...
    def colorGroups(self, size, group_set):
        colors = [
            '#B71C1C', '#9C27B0', '#3F51B5', '#00897B', '#4CAF50', '#CDDC39',
            '#FFEB3B', '#FF9800'
        ]
        overlap_colors = [
            'background: #FF4081;', 'background: #FF3D00;',
            'background: #76FF03;'
        ]
        for g_set in group_set:
            for cords in g_set:
                ...
                if btn.styleSheet() in overlap_colors:
                    btn.setStyleSheet(
                        overlap_colors[overlap_colors.index(btn.styleSheet()) +
                                       1])
                else:
